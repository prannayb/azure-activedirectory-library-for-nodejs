...
var AccessTokenMap = constants.AccessTokenMap;
...
/**
 * Given a decoded accessToken off the wire, this function extracts the values that
 * ADAL commonly returns to callers and translates the names to more user
 * friendly names.
 * @param  {Object} accessToken A decoded access token.
 * @return {Object}             The set of extracted values with their new names.
 */
OAuth2Client.prototype._extractAccessTokenValues = function(accessToken) {
  var extractedValues = {};
  mapFields(accessToken, extractedValues, AccessTokenMap);
...
/**
 * Parses the value of the access token.
 * @param  {string} encodedAccessToken  An unencrypted JWT token.
 * @return {object}                     returns the decoded AccessToken or undefined.
 */
OAuth2Client.prototype.parseAccessToken = function(encodedAccessToken) {
  var crackedToken = this._crackJwt(encodedAccessToken);
  if (!crackedToken) {
    return;
  }
  ...
  try {
    ...
    parsedToken = JSON.parse(base64Decoded);
  } catch(err) {
    this._log.warn('The returned id_token could not be decoded: ' + err.stack);
    return;
  }

  var accessToken = this._extractAccessTokenValues(parsedToken);
  var idToken = this._extractIdTokenValues(parsedToken);
  _.extend(accessToken, idToken);
  return accessToken;
};
